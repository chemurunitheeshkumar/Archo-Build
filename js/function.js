(function ($) {
	...
	/* YouTube Popup Video */
	if ($('.popup-youtube, .lightbox-image').length) {
		$('.popup-youtube, .lightbox-image').magnificPopup({
			type: 'iframe',
			mainClass: 'mfp-fade',
			removalDelay: 160,
			preloader: false,
	...
	/* =============================================
	   SITE-WIDE IMAGE POPUP (Magnific Popup)
	   Auto-applies to project, portfolio, blog, gallery images
	============================================= */
	$window.on('load', function () {

		// All image figure selectors to make clickable
		var imageSelectors = [
			...
			'.industrial-vertical-item figure',
			'.what-we-do-mosaic figure',
			'.mosaic-main figure',
			'.mosaic-sub figure',
			'.main-frame figure',
			...
			'.team-image figure',
			'.completed-project-image figure',
			'.ongoing-project-image figure',
			'.service-item-image figure',
			'.blueprint-visual .main-frame figure',
			'.residential-gallery figure',
		...
		$(imageSelectors).each(function () {
			...
			if ($img.length && !$fig.parent().hasClass('popup-img-trigger')) {
				var imgSrc = $img.attr('src');
				var imgAlt = $img.attr('alt') || '';
				if (imgSrc && !$fig.parent().is('a')) {
					$fig.wrap('<a class="popup-img-trigger" href="' + imgSrc + '" title="' + imgAlt + '"></a>');
				}
			}
		});

		// Initialize popup on all wrapped images - with gallery grouping per section
		if ($('.popup-img-trigger').length) {

			// Sections with gallery navigation (swipe between images)
			var gallerySections = '.our-projects, .completed-projects-section, .ongoing-projects-section, .residential-projects-section, .commercial-projects-section, .project-gallery-section';

			$(gallerySections).each(function () {
				$(this).find('.popup-img-trigger').magnificPopup({
					type: 'image',
					gallery: {
						enabled: true,
			...
			// All other standalone images (not in a gallery section)
			var galleryTriggerSelector = gallerySections.split(',').map(function (s) {
				return s.trim() + ' .popup-img-trigger';
			}).join(', ');

			$('.popup-img-trigger').not(galleryTriggerSelector).magnificPopup({
				type: 'image',
				image: {
					titleSrc: 'title',
					cursor: 'mfp-zoom-out-cur'
	...
	if ($('.active-sticky-header').length) {
		...
		$window.on("scroll", function () {
			...
			$("header.active-sticky-header .header-sticky").toggleClass("active", (fromTop > headerHeight));
		});
	}
	...
	if ($('.testimonial-marquee').length) {
		const testimonial_marquee = new Swiper('.testimonial-marquee', {
			slidesPerView: 1,
			spaceBetween: 30,
			loop: true,
			speed: 1000, // Standard transition speed
			autoplay: {
				delay: 3000, // Wait 3 seconds before next slide
				disableOnInteraction: false,
			...
			breakpoints: {
				...
				1400: {
					slidesPerView: 3, // Keep 3 for better readability
				}
			}
		});
	}
